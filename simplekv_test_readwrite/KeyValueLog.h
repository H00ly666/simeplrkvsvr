...
public:
    KeyValueLog() {}
    KeyValueLog(const std::string &path, const int &id, const int &fd, const size_t &globalOffset, char *MapBuffer, const size_t mapFileSize) :
        id(id), path(path), current_value_Fd(fd) ,filePosition(0), globalOffset(globalOffset),
        MapBufferPosition(0), MapBuffer(MapBuffer), enlarge(false), currentKeyFileSize(mapFileSize) {
    }
    ...
    void KeyValueLogEnlarge() {
        ...
        //当前大小扩容双倍
        munmap(MapBuffer, currentKeyFileSize);
