...
namespace simplekvsvr {
    ...
    private:
        ...
        KeyValueLog *keyValueLogs[FILE_NUM];
        lru11::Cache<std::string, std::string, std::mutex> LRUCache[FILE_NUM];
        google::dense_hash_map<std::string, KVMapEntry> KVMap[FILE_NUM];

        PMutex MapMutex[FILE_NUM];
        int totalNum = 0;
        bool unEnlarge = true;
    ...
    public:
        ...
        explicit PEngine(const string &path)  {
            ...
            if (access(filePath.data(), 0) != -1) {
                ...
                //初始化 map , value  存储文件 FILE_NUM个文件
                ...
                //FILE_NUM线程读key 加载google_hash_map索引
